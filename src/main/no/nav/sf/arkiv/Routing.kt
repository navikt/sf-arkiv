    ...
    post("/hente") {
        ...
        if (devBypass || containsValidToken(call.request)) {
            ...
            if (requestBody.isEmpty()) {
                call.respond(BadRequest, "Request contains no search parameters, that is not allowed")
            }
            if (!requestBody.hasValidDokumentDato()) {
                call.respond(BadRequest, "Request contains invalid dokumentdato (correct format is empty or yyyy-MM-dd)")
            }
